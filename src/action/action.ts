...
OBR.onReady(async () => {
  const start = async () => {
    ...
    await setUpActionPopover();
    updateActionTheme(await OBR.theme.getTheme());
  };
...
async function setUpInputs() {
  ...
  for (const actionInput of actionInputs) {
    ...
    (
      document.getElementById(actionInput.id) as HTMLInputElement
  ...
  if (retrievedValue) {
    ...
    if (actionInput.type === "CHECKBOX") {
      if (
        ...
        (document.getElementById(actionInput.id) as HTMLInputElement).checked =
          value;
      } else {
        (document.getElementById(actionInput.id) as HTMLInputElement).checked =
          false;
      ...
      if (
        ...
        (document.getElementById(actionInput.id) as HTMLInputElement).value =
          String(value);
      } else {
        (document.getElementById(actionInput.id) as HTMLInputElement).value =
          String(0);
