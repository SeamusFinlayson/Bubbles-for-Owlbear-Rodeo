      ...
      async (itemsFromCallback) => {
        ...
        for (let i = 0; i < imagesFromCallback.length; i++) {
          if (i > itemsLast.length - s - 1) {
            ...
            !(
              itemsLast[i + s].position.x ===
                imagesFromCallback[i].position.x &&
              itemsLast[i + s].position.y ===
                imagesFromCallback[i].position.y &&
              itemsLast[i + s].visible === imagesFromCallback[i].visible &&
              JSON.stringify(
                itemsLast[i + s].metadata[getPluginId("metadata")],
              ) ===
                JSON.stringify(
                  imagesFromCallback[i].metadata[getPluginId("metadata")],
                ) &&
              JSON.stringify(
                itemsLast[i + s].metadata[getPluginId("name-tag")],
              ) ===
                JSON.stringify(
                  imagesFromCallback[i].metadata[getPluginId("name-tag")],
                )
            )
          ) {
            //update items
        ...
        await createNameTagBackgrounds(changedItems);
      },
    );
...
async function createNameTagBackgrounds(itemsWithNameTags: Item[]) {
  const nameTagBackgrounds: Item[] = [];
  if (await OBR.scene.isReady()) {
    for (let i = 0; i < itemsWithNameTags.length; i++) {
      nameTagBackgrounds.push(
        ...createNameTagBackground(
          itemsWithNameTags[i],
          await OBR.scene.local.getItemBounds([
            getNameTagTextId(itemsWithNameTags[i].id),
          ]),
        ),
      );
    }
...
async function createAttachments(
  ...
  const { origin, bounds } = getOriginAndBounds(item, dpi);
  ...
  function createLimitedHealthBar() {
    ...
    if (maxHealth <= 0) {
      addHealthItemAttachmentsToDeleteList(item.id);
      return;
    }
...
function getOriginAndBounds(item: Image, dpi: number) {
  // Determine bounds
...
function addHealthItemAttachmentsToDeleteList(itemId: string) {
  deleteItemsArray.push(
    `${itemId}health-background`,
    `${itemId}health`,
    `${itemId}health-label`,
...
function addArmorItemAttachmentsToDeleteList(itemId: string) {
  deleteItemsArray.push(`${itemId}ac-background`, `${itemId}ac-label`);
}

function addTempHealthItemAttachmentsToDeleteList(itemId: string) {
  deleteItemsArray.push(
    `${itemId}temp-hp-background`,
    `${itemId}temp-hp-label`,
  );
...
function addNameTagItemAttachmentsToDeleteList(itemId: string) {
  deleteItemsArray.push(
    `${itemId}name-tag-background`,
    `${itemId}name-tag-text`,
  );
  ...
  for (const actionInput of actionInputs) {
    ...
    if (retrievedValue) {
      ...
      if (actionInput.type === "CHECKBOX") {
        ...
        if (
          checkForSettingsValueChange(actionInput.id, value) &&
...
async function updateNameTagContextMenuIcon() {
  if (settings.nameTags) {
    ...
    OBR.contextMenu.create({
      ...
      embed: {
        url: "/src/name-tags/nameTag.html",
        height: 60,
      },
